...
export const CalendarConverter: React.FC = () => {
  ...
  const convertDate = () => {
    ...
    try {
      ...
      const parts = new Intl.DateTimeFormat('fr-FR-u-ca-islamic-umalqura', {
        day: 'numeric',
        month: 'long',
        year: 'numeric'
      }).formatToParts(gDate);
      ...
      setHijriDate('Erreur de conversion');
    }
  };
    ...
    <div className="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8 safe-area-pt pb-24 border-none">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Convertisseur Grégorien vers Hégirien</p>
        </div>
      </div>
      ...
      <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800/50 rounded-2xl p-4 mb-8">
        <p className="text-sm text-emerald-800 dark:text-emerald-200">
          Ce convertisseur utilise le calendrier islamique d'Umm al-Qura. Il vous permet d'identifier les dates propices (comme le milieu du mois lunaire) pour vos pratiques spirituelles.
        </p>
      </div>
      ...
      {hijriDate && (
        ...
        >
          ...
          <h3 className="text-emerald-300 font-bold uppercase tracking-widest text-xs mb-2">Correspondance Hégirienne</h3>
          
          <div className="text-3xl sm:text-4xl text-white mb-2 leading-relaxed font-bold tracking-tight">
            {hijriDate}
          </div>
          
          <p className="text-emerald-400/80 text-sm mt-4">
            Mois de {islamicMonth}
          </p>
        </motion.div>
      )}
    </div>
